...
# Why PhaME_m?
Since PhaME has not been updated for a long time, the existing versions (precompiled, development, and docker versions) have some issues on installation or running. In PhaME_m, all of these issues are solved. Following the installation guide below, the PhaME can be successfully installed and run on a local machine. 
